...
int bq_router(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    }else if(!strcasecmp(ptr->cmd,BQ_GET_TOPIC_LIST)){
        result = bq_get_topic_list(serverPtr,ptr);
    }else{
...
static int bq_empty_baseServer(BaseServerPtr serverPtr,NetMessagePtr ptr){
    return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_EMPTY_BASESERVER),BQ_EMPTY_BASESERVER_MSG);

}

static int bq_unknow_cmd(BaseServerPtr serverPtr,NetMessagePtr ptr){
    return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_UNKNOW_CMD),BQ_UNKNOW_CMD_MSG,ptr->cmd);
}

static int bq_add(BaseServerPtr serverPtr,NetMessagePtr ptr){
    char * param = getExtraParam(ptr,"topic_name");
    if(param == NULL){
        return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_EMPTY_TOPIC_NAME),BQ_EMPTY_TOPIC_NAME_MSG);
    }
    TopicPtr tPtr = useTopic(serverPtr,param);

    if(tPtr == NULL){
        freeString(&param);
        return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_UNKNOW_TOPIC),BQ_UNKNOW_TOPIC_MSG,param);
    }else{
        freeString(&param);
        ...
        if(mPtr == NULL){
            return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_BUILD_MESSAGE_FAILED),BQ_BUILD_MESSAGE_FAILED_MSG);
        }
        
        addMessage(tPtr,mPtr,delay);
        return setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_ADD_SUCCESS),BQ_ADD_SUCCESS_MSG);
    }
...
static int bq_get(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    if(tPtr == NULL){
        ...
        if(mPtr == NULL){
            setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_NO_READY_MESSAGE),BQ_NO_READY_MESSAGE_MSG,param);
        
            freeString(&param);
            return 0;
        }else{
            //TODO client cmd undefined now 
            setNetMessageSendData(ptr,buildErrorCode(BQ_SUCCESS_MARK,BQ_GET_SUCCESS),RESPONSE_OK,mPtr->data,mPtr->length);
...
static int bq_sleep(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    if(tPtr == NULL){
        ...
        if(param == NULL){
            return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_EMPTY_MESSAGE_ID),BQ_EMPTY_MESSAGE_ID_MSG);
        }
        int64 messageId  = atoll(param);
        freeString(&param);
        int code  = sleepMessage(tPtr,messageId);
        if(code == -1){
            return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_UNKNOW_MSG),BQ_UNKNOW_MSG_MSG,messageId);
        }else{
            return setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_SLEEP_SUCCESS),BQ_SLEEP_SUCCESS_MSG, messageId);
        }
    }
} 
...
static int bq_reuse(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    if(tPtr == NULL){
        ...
        if(code == -1){
            ...
            return setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_REUSE_SUCCESS),BQ_REUSE_SUCCESS_MSG, messageId);
        }
    }
} 
...
static int bq_del(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    if(tPtr == NULL){
        ...
        if(code == -1){
            ...
            return setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_DEL_SUCCESS),BQ_DEL_SUCCESS_MSG, messageId);
        }
    }
} 
...
static int bq_wakeup(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    if(tPtr == NULL){
        ...
        if(code == -1){
            ...
            return setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_WAKEUP_SUCCESS),BQ_WAKEUP_SUCCESS_MSG, messageId);
        }
    }
} 
...
static int bq_add_topic(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    char * param =NULL;
    param = getExtraParam(ptr,"topic_name");
    if(param == NULL){
        return setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_EMPTY_TOPIC_NAME),BQ_EMPTY_TOPIC_NAME_MSG);
    }
    ...
    if(code == -1){
        int result = setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_EXISTS_TOPIC_NAME),BQ_EXISTS_TOPIC_NAME_MSG,param);
        freeString(&param);
        return result;
    }else{
        UpdateSubscribeAfterAddTopic(serverPtr->appServer->subscribeServer,param);
        int result =  setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_ADD_TOPIC_SUCCESS),BQ_ADD_TOPIC_SUCCESS_MSG, param);
        freeString(&param);
        return result;
    }
...
static int bq_del_topic(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ...
    if(code == -1){
        int result = setNetMessageError(ptr,RESPONSE_ERROR,buildErrorCode(BQ_ERRORS_MARK,BQ_UNKNOW_ERROR),BQ_UNKNOW_ERROR_MSG);
        freeString(&param);
        return result;
    }else{
        UpdateSubscribeAfterRemoveTopic(serverPtr->appServer->subscribeServer,param);
        int result =  setNetMessageError(ptr,RESPONSE_OK,buildErrorCode(BQ_SUCCESS_MARK,BQ_DEL_TOPIC_SUCCESS),BQ_DEL_TOPIC_SUCCESS_MSG, param,code);
        freeString(&param);
        return result;
    }
}
static int bq_clean_topic(BaseServerPtr serverPtr,NetMessagePtr ptr){
    setNetMessageError(ptr,RESPONSE_OK,0, "the cmd unsupport now");
} 

static int bq_get_topic_list(BaseServerPtr serverPtr,NetMessagePtr ptr){
    ListNodePtr start = getListHeader(serverPtr->topicList);
    ListNodePtr end = getListEnd(serverPtr->topicList);
    char* buf= allocMem(TOPIC_LIST_BUF_SIZE);
    int writedLength = 0,leavLength =0,write;
    TopicPtr tptr = nextFromList(&start,end,NULL,NULL);
    while(tptr!= NULL){

        if(writedLength<TOPIC_LIST_BUF_SIZE){
            write = snprintf(buf+writedLength,TOPIC_LIST_BUF_SIZE-writedLength,"ID:%" PRId64 " %s\n",tptr->topicId,tptr->topicName);
            writedLength += write;
        }else{
            break;
        }
    ...
    if(writedLength == 0){
        char* string = " no topic saved in server!";
        strcpy(buf,string);
