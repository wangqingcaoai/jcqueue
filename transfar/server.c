...
makeServerSocket(char *host, char *port)
{
    ...
    r = sd_listen_fds(1);
    if (r < 0) {
        return addLog(LOG_WARNING,"sd_listen_fds"), -1;
    }
    if (r > 0) {
        if (r > 1) {
            addLog(LOG_WARNING,"inherited more than one listen socket;ignoring all but the first");
            r = 1;
        }
        ...
        if (r < 0) {
            errno = -r;
            addLog(LOG_WARNING,"sd_is_socket_inet");
            return -1;
        }
        if (!r) {
            addLog(LOG_WARNING,"inherited fd is not a TCP listen socket");
            return -1;
        }
        return fd;
    }
//初始化数据
    ...
    if (r == -1)
      return addLog(LOG_WARNING,"getaddrinfo()"), -1;

    for(ai = airoot; ai; ai = ai->ai_next) {
      //对socket进行设置
      fd = socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol);
      if (fd == -1) {
        addLog(LOG_WARNING,"socket()");
        continue;
      }

      flags = fcntl(fd, F_GETFL, 0);
      if (flags < 0) {
        addLog(LOG_WARNING,"getting flags");
        close(fd);
        continue;
      }
      ...
      if (r == -1) {
        addLog(LOG_WARNING,"setting O_NONBLOCK");
        close(fd);
        continue;
      }
      ...
      if (r == -1) {
        addLog(LOG_WARNING,"setting SO_REUSEADDR on fd %d", fd);
        close(fd);
        continue;
      }
      r = setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &flags, sizeof flags);
      if (r == -1) {
        addLog(LOG_WARNING,"setting SO_KEEPALIVE on fd %d", fd);
        close(fd);
        continue;
      }
      r = setsockopt(fd, SOL_SOCKET, SO_LINGER, &linger, sizeof linger);
      if (r == -1) {
        addLog(LOG_WARNING,"setting SO_LINGER on fd %d", fd);
        close(fd);
        continue;
      }
      r = setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &flags, sizeof flags);
      if (r == -1) {
        addLog(LOG_WARNING,"setting TCP_NODELAY on fd %d", fd);
        close(fd);
        continue;
      }
